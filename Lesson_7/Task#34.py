...
#     **Вывод:** Парам пам-пам
data_list = "паара-ра-рам рам-пам-папам па-раа-па-да"
def winni_help(data):
    phrase_list = data.split(" ")
    collection = 'аеёиоуыэюя'
    list1 = []
    for i in phrase_list:
        count = 0
        for j in range(len(i)):
            if i[j] in collection:
                count += 1
        list1.append(count)
    print(list1)
    num_match = list1.count(list1[0])
    if num_match == len(list1):
        return 'Парам пам-пам'
    else:
        return 'Пам парам'
print(winni_help(data_list))